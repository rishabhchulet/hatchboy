...
feature "Confirm password" do
  ...
  background do
    @not_confirmed_user = FactoryGirl.create(:not_confirmed_account)
    @confirmed_user = FactoryGirl.create(:account)
  end
  ...
  context "with not confirmed user" do
    ...
    scenario "should change confirmed_at field on confirmation" do
      ...
      expect{ visit account_confirmation_path(:confirmation_token => "valid_token") }.to change{ @not_confirmed_user.reload.confirmed_at }.from(nil)
    end

    scenario "should show successfull confirmation alert" do
      Devise.token_generator.stub(:digest).and_return(@not_confirmed_user.confirmation_token)
      visit account_confirmation_path(:confirmation_token => "valid_token")
    ...
    scenario "should display form to resend confirmation instructions" do
      visit account_confirmation_path
