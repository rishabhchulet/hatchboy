module Hatchboy
  module ReportsFilters
    class WorkLogsFilter
      ...
      def from_date date
        self.class.new @scope.where("work_logs.created_at > ?", date)
      end

      def in_date date
        self.class.new @scope.where("DATE(work_logs.created_at) = ?", date)
      end

      def in_date_range from, to
        self.class.new @scope.where("DATE(work_logs.created_at) BETWEEN ? AND ?", from, to)
      end
      ...
      def group_by_date_field field 
        self.class.new @scope.select("DATE_TRUNC('#{field}', work_logs.created_at) AS g_created_at").group("g_created_at").order("g_created_at")
      end
